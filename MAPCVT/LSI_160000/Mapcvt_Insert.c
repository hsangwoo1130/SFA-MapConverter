...
/** [Start] Follow variables are just used by Digital Server */
	pid_t pid;
	int fd;
	int nRets;
	char stProcFileName[256];
	/** [end] **/

	printf("Inkless D/A MAPCVTmgr started !!\n");

	/*  setpgrp(getpid(),1); */     /* Become Session leader */
	setpgrp(); /* 2012-03-13 */
...
**/
			...
			case 0:
				...
				exit(0);
			default:
				/** while(waitpid(0, &nRets, 0) <= 0) ;   **/
				if (pthread_create(&lThread, NULL, pThreadWork, (void*)pid) != 0)
				{
#ifdef _DEBUG_LOG
					printf("Thread Creation error[%d:%s]\n", errno, sys_errlist[errno]);
#endif
				}
				break;
			}  /* end of switch */
...
/** end of Modification **/
{
	...
	/*int nFirst200_flag; */ /*2006.08.08  2009-02-11 주석처리*/
...
/** you must use waferid in mapfile for real waferid          **/
	...
	memcpy(MapBody.stMID, tmpWaferId, 7);;
	...
	************************/
	...
	nFirstDie = 0; /*2007-10-01 first die 를 초기화 */

	pTemp = strstr(stTok, "&");

	if (pTemp != NULL)
	{
		printf("strtok #3 - %s ,%d====================\n", pTemp + 1, atoi(pTemp + 1));
		nFirstDie = atoi(pTemp + 1);
	}

	/* 2009-02-11 주석처리

	pTemp = strstr(stTok, "200");
	...
	*/
	...
	printf("STROTOK CHEKC #1\n");


	/*	if(strcmp(pTemp, "200") == 0 )
			nFirst200_flag = 1;
	*/
	/* 2006.08.08 end */
	...
	{
		/*** Read Part ID ***/
		printf("POINT2 STRTOK : %s\n", stTok);
		if (nCount == 2) memcpy(stPartId, stTok, strlen(stTok));
		/** Data Validation **/
		if (stPartId == NULL)
		{
			fclose(fpRead);
	...
	while ((stTok = strtok(NULL, DELIMITER4)) != NULL)
	{
		...
		if (nCount == 5)
		{
			...
			/** Bin Flag  **/
  /* Modified by msyun for implementing FTER 490 Temporary */
			nBIN_No = atoi(stTok);
...
#ifdef _DEBUG_MODE
				printf("[1]stTok[%s]==> cTemp[%c]\n", stTok, cTempBin);
#endif
				/*
				}
				else if ( atoi(stTok) == 2 )
				{
					cTempBin = 2 + '0' ;
				*/
#ifdef _DEBUG_MODE
	...
	*/
	...
	printf("\n\n ============  EDGE BIN Process While End \n");

	/* 2006.08.08 start */
	/*  2009-02-11 주석 처리
	if ( nFirst200_flag == 1)
	{
		pntFirstDie.sXpos = 200;
		pntFirstDie.sYpos = 200;
...
#ifdef _DEBUG_MODE
	printf("\n ===================== F_D_X = [%d] , F_D_Y = [%d] =========================\n\n", pntFirstDie.sXpos, pntFirstDie.sYpos);
