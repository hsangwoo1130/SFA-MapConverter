...
extern int nInsertDb_MES();		/* Added by swH 2026.01.20*/
int 	Error_Log();
void 	Get_Date();

extern int DBReadFileSave_V2(); /* Added by swH 2026.01.20*/

/* ISH Append 20251205 */
#define TRUE  1
#define FALSE 0

/* modified by swH 26/01/20 */

/*
#define MAP_MANUAL_CONVERT_FILE_NAME "/mapap/inkless/Wafer_bin_conv_list.cfg"
int  GetWaferBinListFromFile(char* szWaferID, char* szBinListInfo);
*/

//#define MAP_MANUAL_CONVERT_FILE_NAME "/home/inkless/bin/Wafer_bin_conv_list_v2.cfg"
#define MAP_MANUAL_CONVERT_FILE_NAME "/mapap/inkless/Wafer_bin_conv_list_v2.cfg"
int  GetWaferBinListFromFile();

//char GetBinValue(char* gszBinListInfo, int nBinNo, char cCurValue);

#define DB_INI_FILE_NAME "/mapap/inkless/bin/config/DBUserSFA.ini"
//#define DB_INI_FILE_NAME "/home/inkless/bin/DBUserSFA.ini"

char gszBinListInfo[256];
int gbReadBinList = FALSE;

int  SetBinValueArray(char* gszBinListInfo, char* szConvBin, int nBufSize);
char GetBinValueArray(char* gszConvBin, int nBinNo, char cCurValue);
char gszConvList[256]; /* Conversion Buffer */

/* Added by swH 26/02/09 */
/* DB Connect Information */
char gszUsername[50];
char gszUsername_MES[50];
char gszPasswd[50];
char gszPasswd_MES[50];
char gszDBName[200];
char gszDBName_MES[200];
...
int* argv[];
{
	...
	/* modified by swH 26/01/20 */
	if (DBReadFileSave_V2() != 0)
	{
		printf("Failed to load bin config from database. Using existing config file.\n");
	}
	gbReadBinList = GetWaferBinListFromFile();
	if (gbReadBinList == TRUE)
	{
		Error_Log(InsertGood, gszBinListInfo);    // write log file 
	}
...
int main()
{
	...
	memset(gszUsername, '\0', sizeof(gszUsername));
	memset(gszPasswd, '\0', sizeof(gszPasswd));
	memset(gszUsername_MES, '\0', sizeof(gszUsername_MES));
	memset(gszPasswd_MES, '\0', sizeof(gszPasswd_MES));
	memset(gszDBName, '\0', sizeof(gszDBName));
	...
	case 0:                /* The Child process (daemon) comes here. */
		...
		nRetValue = GetPrivateProfileStringHP("INKLESS", "DBUSER", gszUsername, DB_INI_FILE_NAME);
		nRetValue = GetPrivateProfileStringHP("INKLESS", "DBPASS", gszPasswd, DB_INI_FILE_NAME);
		nRetValue = GetPrivateProfileStringHP("INKLESS", "DBUSING", gszDBName, DB_INI_FILE_NAME);
		nRetValue = GetPrivateProfileStringHP("MES", "DBUSER", gszUsername_MES, DB_INI_FILE_NAME);
		nRetValue = GetPrivateProfileStringHP("MES", "DBPASS", gszPasswd_MES, DB_INI_FILE_NAME);
		nRetValue = GetPrivateProfileStringHP("MES", "DBUSING", gszDBName_MES, DB_INI_FILE_NAME);
		...
		for (;;)
		{
			...
			if (Rdmsg < 0)
				...
				case -1:
					...
					sprintf(stCvtfile, "%s%s", LSI_MAP_DATA_DIRECTORY, MsgQ_WaferList.msgData.Wafer_List[0]);
					stTok = strtok(MsgQ_WaferList.msgData.Wafer_List[0], DELIMITER5);
					sprintf(stMovefile, "%s%s.REQ", LSI_MAP_DATA_DIRECTORY, stTok);
				...
				case  0:
					...
					if (DBReadFileSave_V2() != 0)
					{
						...
						printf("Failed to load bin config from database. Using existing config file.\n");
#endif
						Error_Log(InsertErr, "DBReadFileError : Fail to load bin config from database.");
					}
					...
					{
						Error_Log(InsertGood, gszBinListInfo);    // write log file 
						nRetValue = SetBinValueArray(gszBinListInfo, gszConvList, 256);
					}
...
/** Because Run_id in Map file is able to be different from Run_id in *.REQ file **/
	...
	sprintf(stCvtfile, "%s%s", LSI_MAP_DATA_DIRECTORY, stWafer_List->Wafer_List[0]);
	...
	{
		memset(stCvtfile, '\0', 256);
		sprintf(stCvtfile, "%s%s", LSI_MAP_DATA_DIRECTORY, stWafer_List->Wafer_List[nLoop]);
...
/** end of Modification **/
{
	...
	char	cRetValue;
	char	szBinList[256];
	//int  	bReadBinList = TRUE;

	if (gbReadBinList == TRUE)
	{
		strcpy(szBinList, gszBinListInfo);
#ifdef _DEBUG_MODE
		printf("DEBUG: Loaded BinList = [%s]\n", szBinList);
#endif
	}
	else
	{
		memset(szBinList, 0, sizeof(szBinList));
	}

	char tmpWaferId[15];  /** Modified by stainles 2001.04.19 **/

	/* 2005.04.18 cmg modify */
	int nF_D_X30_Flag;
	int nF_D_Y30_Flag;
	int nF_D_X100_Flag;
	int nF_D_Y100_Flag;

	/* 2005.09.22 cmg modify */
	int nF_D_30;
	int nF_D_100;

	int nFirstDie; /* 2007-10-01 FIRST DIE 를 설정한다 */
	char* pTemp; /*2006.08.08 */
...
/* Read Part Id */
	...
	for (nLoopRow = 0; nLoopRow < 600; nLoopRow++)
	{
		for (nLoopCol = 0; nLoopCol < 600; nLoopCol++)
		{
			stBinArray[nLoopCol][nLoopRow] = NO_DIE_MK;
		}
	}

	/* ISH Read Change Bin List 2025/12/05 */
	/***
	gbReadBinList = GetWaferBinListFromFile(MapBody.stMID, szBinList);
	if (gbReadBinList == TRUE)
	{
		Error_Log(InsertGood, szBinList);    // write log file
	} ***/
	...
	while ((stTok = strtok(NULL, DELIMITER4)) != NULL)
	{
		...
		if (nCount == 5)
		{
			...
			/* ISH Bin Convert 2025/12/05 */
			if (gbReadBinList == TRUE)
			{
				cRetValue = GetBinValueArray(gszConvList, atoi(stTok), cTempBin);
#ifdef _DEBUG_MODE
				if (cRetValue != cTempBin)
					printf("Conv Result [%d] => [%c]\n", atoi(stTok), cRetValue);
#endif		
				cTempBin = cRetValue;

				/* modified by swH 26/01/20 */
				/* cRetValue = GetBinValue(szBinList, atoi(stTok), cTempBin);
				cRetValue = GetBinValue(szBinList, atoi(stTok), cTempBin);
				printf("Conv Result [%c->%c]\n", cTempBin, cRetValue);
				cTempBin = cRetValue; */
			}
		...
		else if (!memcmp(stTok, "X", 1))
		{
			while ((stTok = strtok(NULL, DELIMITER4)) != NULL)
			{
				...
				if (nCount == 1)
				{
					...
					{
						nF_D_X30_Flag = 1;
						printf("====== X30 Flag = [%d] ======", nF_D_X30_Flag);
					}
					if (nTemp_X == 100)
					{
						nF_D_X100_Flag = 1;
						printf("====== X100 Flag= [%d] ======", nF_D_X100_Flag);
					}
				}
				/** Y Position **/
				if (nCount == 3)
				{
					...
					{
						nF_D_Y30_Flag = 1;
						printf("====== Y30 Flag = [%d] ======", nF_D_Y30_Flag);
					}
					if (nTemp_Y == 100)
					{
						nF_D_Y100_Flag = 1;
						printf("====== Y100 Flag= [%d] ======", nF_D_Y100_Flag);
...
*/
	...
	if (nFirstDie != 0)
	{
		printf("FIRSTDIE SETTTING : %d\n", nFirstDie);
		pntFirstDie.sXpos = nFirstDie;
		pntFirstDie.sYpos = nFirstDie;
	...
	else
	{
		...
		if (nF_D_30 == 1)
		{
			pntFirstDie.sXpos = 30;
			pntFirstDie.sYpos = 30;
			nF_D_30 = 0;
			nF_D_100 = 0;
		}
		else
		{
			if (nF_D_100 == 1)
			{
				pntFirstDie.sXpos = 100;
				pntFirstDie.sYpos = 100;
				nF_D_30 = 0;
				nF_D_100 = 0;
			}
			else
	...
	/* Added by swH 2026.01.20 */
	nErrorCode_MES = nInsertDb_MES(&MapBody);
	...
	if (nErrorCode != EXE_OK)
	{
		...
		{
			/* Inkless DB insert fail */
#ifdef _DEBUG_MODE
			printf("INKLESS DB Inserting Error With sqlcode=[%d]\n", nErrorCode);
#endif
			sprintf(stErrorCode, "INKLESS ORACLE ERROR: %d", nErrorCode);

			/* MES DB Check */
			if (nErrorCode_MES != EXE_OK)
			{
#ifdef _DEBUG_MODE
				printf("MES DB Inserting Error With sqlcode=[%d]\n", nErrorCode_MES);
#endif
				sprintf(stErrorCode, "ORACLE INSERT ERROR: %d", nErrorCode_MES);
				Error_Log(InsertErr, stErrorCode);
				return (InsertErr);
			}
			else
			{
#ifdef _DEBUG_MODE
				printf("MES DB Insert Success\n");
#endif
				/* INKLESS fail but, MES insert success -> return InsertGood */
				sprintf(stErrorCode, "INKLESS ORACLE ERROR: %d", nErrorCode);
				return (InsertGood);
			}
		}
	...
	else
	{
		if (nErrorCode_MES != EXE_OK)
		{
			/* INKLESS success but, MES fail -> return InsertGood */
#ifdef _DEBUG_MODE
			printf("MES DB Inserting Error With sqlcode=[%d]\n", nErrorCode_MES);
#endif
			sprintf(stErrorCode, "MES ORACLE ERROR: %d", nErrorCode_MES);
			Error_Log(InsertErr, stErrorCode);
		}
	}
	...
	/*
	if (nErrorCode_MES != EXE_OK) {
		printf("MES Data Inserting Error With ErrorCode=[%d]\n", nErrorCode_MES);
	}
...
/*int GetWaferBinListFromFile(char* szWaferID, char* szBinListInfo)*/
int GetWaferBinListFromFile() /* modified by swH 26/01/20 */
{
	FILE* fpBinInfo;
	...
	/*char szReadWaferID[256];*/ /* modified by swH 26/01/20 */
	char szReadBinList[512];

	sprintf(stBinInfoFile, MAP_MANUAL_CONVERT_FILE_NAME);	/* Wafer Bin Info*/

	/*memset(szBinListInfo, 0x0, sizeof(szBinListInfo));*/ /* modified by swH 26/01/20 */
			...
			if (strcmp(stTok, "WAFERSPEC") == 0)
			{
				while ((stTok = strtok(NULL, "= \r\n")) != NULL)
				{
					...
					if (strcmp(stTok, "BINCONVLIST") == 0)
					{
						...
						/*strcpy(szBinListInfo, szReadBinList);*/
...
//	char szBinList[256];
//
//	strcpy(szBinList, gszBinListInfo); /* for original value save */
//
//
//	if (cCurValue == '1')		
//	{
...
char GetBinValueArray(char* szConvBin, int nBinNo, char cCurValue)
{
	if (cCurValue == '1')         /* GOOD BIN�ΰ�츸ó����(�������) */
	{
		if (szConvBin[nBinNo] == '.') return cCurValue;
		else return szConvBin[nBinNo];
	}
	return cCurValue;
}
...
int  SetBinValueArray(char* gszBinListInfo, char* szConvBin, int nBufSize)
{
	...
	printf(" BIN LIST SIZE = %d", sizeof(szConvBin));

	memset(szConvBin, '.', nBufSize - 1);
	szConvBin[nBufSize - 1] = 0x0;

	strcpy(szBinList, gszBinListInfo);

	printf(">>> MAKE CONV BIN LIST - BINLIST [%s]\n", szBinList);
	if (strlen(szBinList) == 0) return 0; /* Noting Bin List */

	stTokBin = strtok(szBinList, ",:");
	if (stTokBin == NULL) return 0;
	...
	while ((stTokBin = strtok(NULL, ",:")) != NULL)
	{
		...
		{
			szConvBin[nReadBinNo] = stTokBin[0];
			printf("....SET BIN [%d -> %c]\n", nReadBinNo, szConvBin[nReadBinNo]);
		}
	}
