...
setpgrp();
  ...
  switch(fork()) {
	...
	case  0: 
		...
		printf("\nMap_data_task_LSI Process start!! \n");
		Map_data_task();
		exit(0);
		break;
...
void Map_data_task()
{
  ...
  int  nSdmsg;
  	...
  	if((pDir = opendir(LSI_MAP_DATA_DIRECTORY)) == NULL) {
		printf("Directory [%s] open error [%d]\n", LSI_MAP_DATA_DIRECTORY, errno);
		exit(-1);
  	} /* end of if */	

#ifdef _DEBUG_MODE
	printf("Directory : %s\n", LSI_MAP_DATA_DIRECTORY);
#endif
  	...
  	while(1) {
		if( (pDirent = readdir(pDir)) == NULL) {
        	...
        	memset(stBuff,'\0',256);
        	memcpy(stBuff,LSI_MAP_DATA_DIRECTORY,strlen(LSI_MAP_DATA_DIRECTORY));
		...
		if( opendir(pDirent->d_name) != NULL ) { */
			...
			if( (memcmp(strstr(pDirent->d_name, "."), ".REQ", 4)) != 0 ) { 
				continue;
			} else {
	...
	char stFile_List[30][100];
{
	...
	sprintf(stTempfile, "%s%s", LSI_MAP_DATA_DIRECTORY, stReqFile);
        ...
        if ( stat(stTempfile, &statbuf) < 0 )   /* get current file mode */
                fprintf(stderr, "REQ File[%s]: couldn't get current stat \n", stTempfile);
#endif
        if( S_ISREG(statbuf.st_mode) ) {  /** if Directory, Skip **/
            ...
            if (chmod(stTempfile, statbuf.st_mode) < 0 )
                  fprintf(stderr, "REQ File[%s]: couldn't change mode to  -rw-r--r-- \n", stTempfile);
        } /* end of if( S_ISREG(... */  
	...
	**** end of first log insert and inkless flag check .... Do not use from /98/01/08 *****/
	...
	for(nTemp=1; nTemp <= nFileCnt; nTemp++) {
		memset(stTempfile, '\0', 256);
		sprintf(stTempfile, "%s%s", LSI_MAP_DATA_DIRECTORY, stFile_List[nTemp]);
   ...
   REQopen_Error:                     /******  <=== Occur Open Error in REQ File ******/
	...
	/*	memset(stCmd, '\0', 256);
		sprintf(stCmd,"%stransfer_RPY_LSI -r %s -l -h %s %s.RPY",TRANSFER_BIN_DIRECTORY,KH_EDS_DIRECTORY,KH_EDS_SERVER,stRunId);
		system(stCmd);
*/
		...
		for(nLoop=0; nLoop<=nFileCnt; nLoop++) {
			...
			sprintf(stTempfile,"%s%s",LSI_MAP_DATA_DIRECTORY,stFile_List[nLoop]);
...
*/
	...
	sprintf(stTempfile,"%s%s",LSI_MAP_DATA_DIRECTORY,stFile_List[0]);
	sprintf(stRenfile,"%s%s.REF",LSI_MAP_DATA_DIRECTORY,stRunId);
